...
public class GameObject {
	...
	/**
	 * Move this object to position (x, y). <b>It is better to change the x and
	 * y properties of this object instead. Your call to this function will
	 * likely be overriden.</b>
	 * 
	 * @param x
	 * @param y
	 */
	...
	public float[] addQuadVerts(float nVertices[]) {
		float[] totVert = new float[vertices.length + 12];
		
		for (int i = 0; i < vertices.length; i++) {
			totVert[i] = vertices[i];
		}
		
		for (int i = 0; i < 12; i++) {
			totVert[vertices.length + i] = nVertices[i];
		}
		
		return totVert;
	}
	
	public float[] addQuadGraphs(float nGraphicsCoords[]) {
		float[] totGraph = new float[graphicCoords.length + 8];
		
		for (int i = 0; i < graphicCoords.length; i++) {
			totGraph[i] = graphicCoords[i];
		}
		
		for (int i = 0; i < 8; i++) {
			totGraph[graphicCoords.length + i] = nGraphicsCoords[i];
		}
		
		return totGraph;
	}
	
	public int addQuadIndex(int nIndices) {
	...
	/**
	 * Touch screen released event. Can be overriden, no need to call
	 * super.released().
	 */
	...
	public boolean onScreen() {
		if (x > -width / 2 && x < myRoom.getWidth() + width / 2) {
			if (y > -height / 2 && y < myRoom.getHeight() + height / 2) { return true; }
		}
